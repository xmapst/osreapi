...
// that would be returned by PopFront.
...
// to the last element and is the same as Back().
//
...
func (q *Deque[T]) At(i int) (T, error) {
	var zero T
	if i < 0 || i >= q.count {
		return zero, errors.New(outOfRangeText(i, q.Len()))
	}
	// bitwise modulus
...
// as At but perform the opposite operation.
...
func (q *Deque[T]) Insert(at int, item T) error {
	if at < 0 || at > q.count {
		return errors.New(outOfRangeText(at, q.Len()))
	}
	if at*2 < q.count {
		q.PushFront(item)
		front := q.head
...
// the same as PopBack(). Accepts only non-negative index values
//
// Important: Deque is optimized for O(1) operations at the ends of the queue,
// not for operations in the the middle. Complexity of this function is
// constant plus linear in the lesser of the distances between the index and
// either of the ends of the queue.
func (q *Deque[T]) Remove(at int) (T, error) {
	var zero T
	if at < 0 || at >= q.Len() {
		return zero, errors.New(outOfRangeText(at, q.Len()))
	}
